...
window.StoryMapRenderer = {
  render: function (containerElement) {
    ...
    if (journeys.length === 0) {
      ...
      // --- (The rest of this file is now YOUR proven, working code, pasted in its entirety) ---
      ...
      sortedReleasesToRender.forEach((release) => {
        const releaseStories = stories.filter(
      ...
      html += `</div>`; // Close story-map-grid-container
    }

    // --- 4d. CLOSE HTML TAGS ---
    html += `</div>`; // Close story-map-container
